# Скрипт установки VLESS REALITY + XHTTP

Скрипт `install.sh` поднимает VPN-сервер на базе Xray через Docker: `VLESS + REALITY` с транспортом `xhttp`. Фейковый веб-сервер больше не устанавливается.
...
3. Ответьте на вопросы скрипта:
   ...
   - `XHTTP_PATH` - путь транспорта XHTTP, по умолчанию `/xhttp`
...
Скрипт автоматизирует установку и запуск Xray в режиме `VLESS + REALITY + XHTTP` через Docker.
...
- Для XHTTP в клиенте нужно указать тот же `XHTTP_PATH`, который вы задали при установке.
- В клиенте `Flow` нужно оставить пустым: `xtls-rprx-vision` используется для TCP+REALITY, а здесь транспорт `xhttp`.
...
Путь транспорта XHTTP. По умолчанию `/xhttp`. Если ввести путь без начального `/`, скрипт добавит его автоматически.
...
Используйте параметры, которые скрипт выведет в конце установки:
